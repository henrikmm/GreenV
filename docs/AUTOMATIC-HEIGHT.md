...
| Whether the second model's pixels also leave the grass mask, or only vote on cells | `GREENV_MEASUREMENT_STRUCTURE_MODEL_MASK` | `true`. A query model's mask fades a metre onto the grass beside a rail and, out of the mask in every frame, starved a median strip of two thirds of its points; `false` lets it refuse the rail's cells and leave the strip its evidence |
