...
| The probability the second model must give those classes, summed, for a pixel to be a structure | `GREENV_MEASUREMENT_STRUCTURE_FLOOR` | unset, which leaves Verge Studio's 0.5: a majority of the probability, so no single class need win a pixel the model spreads over fence, railing, wall and bannister |
