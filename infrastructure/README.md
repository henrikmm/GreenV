...
| `GREENV_MEASUREMENT_STRUCTURE_MODEL_MASK` | `measurement_structure_model_mask`, default `true` | whether the second model's structure pixels also leave the grass mask with the margin, or only vote on cells; `false` for a query model whose mask fades onto the grass beside a rail |
