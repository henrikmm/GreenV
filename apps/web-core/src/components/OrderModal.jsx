...
/**
 * Criar uma ordem de serviço a partir de um ou mais trechos.
 *
 * As equipes chegam por propriedade em vez de virem de um módulo: na demo são quatro nomes fixos,
 * na versão real vêm da API, e o formulário é o mesmo nos dois casos. Sem lista, o campo de
 * equipe some — melhor do que um seletor vazio que parece quebrado.
 */
export default function OrderModal({ features, teams = [], onSubmit, onClose }) {
      ...
      >
        ...
        <div style={s.body}>
          ...
          <div style={s.formGroup}>
            ...
            <select style={s.select} value={form.team} disabled={teams.length === 0}
              onChange={e => setForm(f => ({ ...f, team: e.target.value }))}>
              <option value="">{teams.length === 0 ? 'Nenhuma equipe cadastrada' : 'Selecionar equipe...'}</option>
              {teams.map(t => (
