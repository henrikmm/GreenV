...
void main() {
  ...
  test(
    'reports invalid credentials without saying which half was wrong',
    () async {
      final auth = subject(
        MockClient(
  ...
  test(
    'separates an unconfigured identity provider from a bad password',
    () async {
  ...
  test('persists the refresh token before anything else uses it', () async {
    ...
    final auth = subject(
      MockClient((_) async => http.Response(tokens(refresh: 'ref-1'), 200)),
      store: store,
    );
  ...
  test(
    'signs out when the session is gone, so the app can ask again',
    () async {
      final store = MemorySessionStore();
      var calls = 0;
      final auth = subject(
    ...
    final auth = subject(
      MockClient((_) async {
        ...
        return http.Response(
          tokens(access: 'tok-$calls', refresh: 'ref-$calls'),
          200,
        );
      }),
      now: () => clock,
  ...
  test('signing out forgets the session on this device', () async {
    final store = MemorySessionStore();
    final auth = subject(
      MockClient((_) async => http.Response(tokens(), 200)),
      store: store,
    );
