...
final class FakeOperationsGateway implements OperationsGateway {
  ...
  Future<List<MeasuredStretch>> sessionStretches(String sessionId) async {
    _maybeFail();
    return stretchRows.where((s) => s.sessionId == sessionId).toList();
  }

  /// Photographs a test can script, keyed by segment index.
  Map<int, List<SampledFrame>> frameRows = const {};

  /// What each photograph measured, keyed by file name.
  Map<String, FrameReadings> readingRows = const {};

  /// Every image the screens actually fetched, so a test can see the sheet reached for them.
  final List<String> imagesFetched = [];

  @override
  Future<List<SampledFrame>> frames(String sessionId, int segmentIndex) async {
    _maybeFail();
    return frameRows[segmentIndex] ?? const [];
  }
  ...
  Future<FrameReadings?> frameReadings(
    ...
    _maybeFail();
    return readingRows[fileName];
  }

  @override
  Future<Uint8List> frameImage(String imageUrl) async {
    _maybeFail();
    imagesFetched.add(imageUrl);
    // A one-pixel PNG: enough for `Image.memory` to decode without a fixture on disk.
